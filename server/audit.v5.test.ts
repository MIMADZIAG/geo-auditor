...
describe("first_paragraph_answer", () => {
  it("passes for article with direct definitional opener ≤150 words", () => {
    ...
    const result = analyzeContentStructure(makePage(html), "article");
    const check = result.checks.find(c => c.id === "first_paragraph_answer");
    expect(check).toBeDefined();
    expect(check!.status).toBe("pass");
  });

  it("returns info (not fail) for product page — not applicable", () => {
    const html = `<html><body>
      <h1>Blue T-Shirt</h1>
      <p>100% cotton, machine washable, available in sizes S-XL.</p>
    </body></html>`;
    const result = analyzeContentStructure(makePage(html), "product");
    const check = result.checks.find(c => c.id === "first_paragraph_answer");
    expect(check).toBeDefined();
    expect(check!.status).toBe("info");
  });

  it("warns when first paragraph is over 150 words", () => {
  ...
  it("warns when no paragraphs exist on article page", () => {
...
describe("intent_blocks", () => {
  it("passes when all 3 intent blocks are present", () => {
    const html = `<html><body>
  ...
  it("returns info (not fail) for product page", () => {
  ...
  it("warns when only 1 intent block found on article", () => {
    const html = `<html><body>
      <h1>Guide</h1>
...
describe("date_signals freshness", () => {
  it("passes for Article schema with recent dates", () => {
  ...
  it("fails for Article schema with dates older than 24 months", () => {
  ...
  it("returns info (not fail) for Product schema with no dates — evergreen", () => {
    const html = `<html><head><script type="application/ld+json">
  ...
  it("warns for Article schema with dates 13-24 months old", () => {
...
describe("eeat external_citations removed", () => {
  it("does NOT include external_citations check in E-E-A-T results", () => {
    const html = `<html><body>
      <p>Author: John Doe</p>
  ...
  it("still includes external_citations in contentStructure", () => {
    const html = `<html><body>
      <h1>Article</h1>
...
describe("js_rendering H1+para source check", () => {
  it("passes when H1 and paragraphs are in HTML source", () => {
    const html = `<html><body>
      <h1>Schema Markup Guide</h1>
  ...
  it("warns when H1 text is only inside a script tag (CSR pattern)", () => {
  ...
  it("check value contains h1InSource and paraInSource fields", () => {
    const html = `<html><body>
      <h1>Test Title</h1>
...
describe("semantic_triples SPO density", () => {
  it("passes for content with high SPO density", () => {
    const html = `<html><body>
      <h1>Schema Markup</h1>
  ...
  it("fails or warns for vague content with no SPO structure", () => {
    const html = `<html><body>
      <h1>Tips</h1>
  ...
  it("value reports percentage-based SPO ratio, not raw count", () => {
    const html = `<html><body>
      <h1>Test</h1>
