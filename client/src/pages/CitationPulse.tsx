...
import {
  ...
  LayoutDashboard, Search, PenLine, FileText, Star, LogOut, ChevronRight, Plus,
} from "lucide-react";
    ...
    <TooltipProvider>
      <div className="flex h-screen overflow-hidden bg-background">
        ...
        <aside className="w-56 shrink-0 flex flex-col border-r border-border/40 bg-card/30">
          ...
          {/* Nav */}
          <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5 flex flex-col">
            {/* Primary CTA */}
            <Link href="/">
          ...
          {user && (
            <div className="px-3 py-4 border-t border-border/30 space-y-2">
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs ${
                ...
                plan === "pro" ? "bg-violet-500/10 text-violet-400 border border-violet-500/20" :
                plan === "starter" ? "bg-primary/10 text-primary border border-primary/20" :
                "bg-muted/40 text-muted-foreground border border-border/30"
              }`}>
                <Star className="w-3.5 h-3.5" />
                <span className="font-semibold">{plan === "free" ? "Free" : plan === "starter" ? "Starter" : plan === "pro" ? "Pro" : "Business"}</span>
              </div>
              {user.name && (
